...
- **Fichier source** `zea_mays_all_sources.json` : script dédié
  `clean_wgs_from_json.py` créé (même regex que le correctif du
  collecteur, pour garder source JSON et collecteur cohérents). Sauvegarde
  automatique en `.bak` avant écriture, trace de l'opération ajoutée dans
  `metadata.cleaning_history`. **À exécuter côté utilisateur** (script
  fourni, pas encore lancé sur le vrai fichier au moment de la rédaction
  de cette note) :
  ```
  python clean_wgs_from_json.py "Data\clean\species\zea_mays_all_sources.json"
  ```
  Total attendu après nettoyage : 112 387 − 1 291 = **111 096**.
