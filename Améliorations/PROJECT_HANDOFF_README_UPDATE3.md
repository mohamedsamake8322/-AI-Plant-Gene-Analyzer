...
### Point restant, mineur

`Trait: Unknown` réapparu sur un candidat (`zma:103647692`) pendant la
vérification visuelle de ce chantier — à confirmer que c'est un fallback
légitime (gène sans traits annotés en base) et non une régression du
bug corrigé en 13.1, via :
```sql
SELECT gene_id, traits FROM genes WHERE gene_id LIKE '%103647692%';
```
