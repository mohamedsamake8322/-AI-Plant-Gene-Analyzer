...
agro_nt_model_lora.gradient_checkpointing_enable()
agro_nt_model_lora.enable_input_require_grads()
...
# 1. Calcul du pos_weight par classe à partir du train set (gestion du déséquilibre)
train_labels = go_label_matrix_final[go_train_idxs]
n_pos = train_labels.sum(axis=0)
n_neg = train_labels.shape[0] - n_pos

# np.clip(..., 1, None) évite la division par 0 si une classe n'a aucun positif dans le train set
pos_weight = torch.tensor(n_neg / np.clip(n_pos, 1, None), dtype=torch.float32).to(device)

print(f"Poids des classes calculés — min: {pos_weight.min():.1f}, max: {pos_weight.max():.1f}, moyenne: {pos_weight.mean():.1f}")

# 2. Initialisation des composants d'entraînement
optimizer = torch.optim.AdamW(
    list(agro_nt_model_lora.parameters()) + list(classification_head.parameters()),
    lr=1e-4,
)

# Remplacement de la loss classique par la version pondérée
criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
scaler = torch.cuda.amp.GradScaler()

print("Optimizer, criterion (pondéré), scaler initialisés !")
# ============================================================
...
optimizer.zero_grad()
# ============================================================
# BLOC 15 — Boucle d'entraînement (avec protection OOM par batch)
# ============================================================
N_EPOCHS = 15
PATIENCE = 3
best_val_f1 = -1
...
for epoch in range(N_EPOCHS):
    ...
    avg_train_loss = train_loss / max(1, len(train_loader) - n_oom_skipped)
    oom_note = f" ({n_oom_skipped} batch(es) OOM ignoré(s))" if n_oom_skipped else ""
    print(f"Epoch {epoch+1}/{N_EPOCHS} — train_loss: {avg_train_loss:.4f}, val_f1_macro: {val_f1:.4f}{oom_note}")
    history.append({"epoch": epoch+1, "train_loss": avg_train_loss, "val_f1_macro": val_f1, "n_oom_skipped": n_oom_skipped})
...
# Diagnostic — distribution des probabilités prédites sur le dernier batch de validation
agro_nt_model_lora.eval()
classification_head.eval()
with torch.no_grad():
    seqs_diag, labels_diag = next(iter(val_loader))
    seqs_diag = [s[:6144].upper() for s in seqs_diag]
    pooled = get_pooled_embedding_trainable(seqs_diag)
    logits = classification_head(pooled)
    probs = torch.sigmoid(logits)

print(f"Probabilité max prédite sur ce batch : {probs.max().item():.4f}")
print(f"Probabilité moyenne : {probs.mean().item():.4f}")
print(f"Nombre de vrais labels positifs dans ce batch : {int(labels_diag.sum().item())} / {labels_diag.numel()}")