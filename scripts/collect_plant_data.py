...
logger = logging.getLogger("collect_plant_data")
if not logger.handlers:
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(ROOT / "logs" / "collect.log", encoding="utf-8")
            if (ROOT / "logs").exists()
...
def collect_geo_data(term: str | None, accessions: list[str] | None, organism: str | None, retmax: int, plants_only: bool) -> list[dict]:
    ...
    if accessions:
        for accession in accessions:
            try:
                ...
                logger.warning("GEO fetch failed for accession %s: %s", accession, exc)
    ...
    if term:
        try:
            ...
            logger.warning("GEO search failed for term %r: %s", term, exc)
    return records
...
def collect_ensembl_data(symbol: str | None, feature_id: str | None, species: str, seq_type: str) -> list[dict]:
    ...
    try:
        ...
        logger.warning("Ensembl fetch failed: %s", exc)
        return []
...
def collect_atlas_data(term: str | None, gene: str | None, species: str, size: int) -> list[dict]:
    ...
    if gene:
        try:
            ...
            logger.warning("Expression Atlas gene profile failed for %r: %s", gene, exc)
    if term:
        try:
            records.extend(collect_expression_atlas.search_experiments(term, species=species, size=size))
        except Exception as exc:
            logger.warning("Expression Atlas search failed for term %r: %s", term, exc)
    return records
...
def merge_metadata(dbpath: Path, gene_key: str, geo_records: list[dict], atlas_records: list[dict]) -> None:
    ...
    if ok:
        ...
        raise RuntimeError(f"Merge failed for gene {gene_key!r}: {msg}")
...
def add_gene_records(records: list[dict], dbpath: Path) -> None:
    for record in records:
        ...
        if ok:
            ...
            logger.warning("%s: %s (skipped, duplicate or invalid)", key, msg)
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info(
        "Collected: GEO=%d, Ensembl=%d, Expression Atlas=%d, NCBI=%d",
        len(geo_records), len(ensembl_records), len(atlas_records), len(ncbi_records),
    )
    ...
    if args.out:
        ...
        logger.info("Wrote combined data to %s", args.out)
    else:
